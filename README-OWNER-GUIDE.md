...
| `lint.py` | Health checks (13 structural + 1 LLM) | `--structural-only`, `--kb internal\|external\|both` | `scripts/state.json` (last lint), `reports/lint-*.md` |
...
| "Lint the wiki", "Run health check" | `wiki-linter` | 13 structural + 1 LLM checks |
...
- **Lint checks**: 13 structural + 1 LLM = 14 total.
