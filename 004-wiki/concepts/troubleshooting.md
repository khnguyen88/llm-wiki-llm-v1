...
Diagnostic procedures for identifying and resolving performance, stability, search, and installation issues in Claude Code. The `/doctor` command provides an automated health check covering installation, settings, MCP configuration, and context usage; `claude doctor` runs the same check from the shell when Claude Code cannot start. ^[raw/document/claude code/claude-code-110-troubleshooting-2026-04-29.md] For installation and login errors specifically, see [[004-wiki/concepts/troubleshoot_install]].
