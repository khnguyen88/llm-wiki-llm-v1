...
Procedures for diagnosing and fixing installation, PATH, permission, binary compatibility, and authentication errors when installing or signing in to Claude Code. For runtime issues after Claude Code is working, see [[004-wiki/concepts/troubleshooting]]. ^[001a-raw/document/claude code/claude-code-111-troubleshoot-install-2026-04-29.md]

## Key Points

- The installer places the binary at `~/.local/bin/claude` (macOS/Linux) or `%USERPROFILE%\.local\bin\claude.exe` (Windows); `command not found` after install means this directory is not on PATH ^[001a-raw/document/claude code/claude-code-111-troubleshoot-install-2026-04-29.md]
- Multiple installations (native, npm `-g`, legacy `~/.claude/local/`, Homebrew cask, WinGet) can conflict; keep only the native install and remove others ^[001a-raw/document/claude code/claude-code-111-troubleshoot-install-2026-04-29.md]
- The installer requires write access to `~/.local/bin/` and `~/.claude/`; use `sudo chown -R $(whoami) ~/.local` if permissions are wrong ^[001a-raw/document/claude code/claude-code-111-troubleshoot-install-2026-04-29.md]
- Claude Code requires at least 4 GB RAM; the Linux OOM killer terminates installs on low-memory servers, fixed by adding 2 GB swap ^[001a-raw/document/claude code/claude-code-111-troubleshoot-install-2026-04-29.md]
- `ANTHROPIC_API_KEY` overrides subscription OAuth and can cause "This organization has been disabled" 403 errors from stale keys; unset it and remove it from shell profiles to use subscription authentication ^[001a-raw/document/claude code/claude-code-111-troubleshoot-install-2026-04-29.md]
