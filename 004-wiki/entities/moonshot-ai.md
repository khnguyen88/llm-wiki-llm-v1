---
title: "Moonshot AI"
summary: "LLM provider available on OpenRouter with automated prompt caching and no cache write costs"
type: entity
sources:
  - raw/document/openrouter/openrouter-070-guides-best-practices-prompt-caching-2026-04-29.md
tags:
  - llm-provider
  - moonshot
  - prompt-caching
created: "2026-05-02T12:00:00Z"
updated: "2026-05-02T12:00:00Z"
confidence: 0.9
provenance: extracted
---

# Moonshot AI

LLM provider available on OpenRouter with automated prompt caching. Cache writes incur no cost, and cache reads are charged at a reduced multiplier of the original input pricing. ^[001a-raw/document/openrouter/openrouter-070-guides-best-practices-prompt-caching-2026-04-29.md]

## Key Facts

- Prompt caching is automated and requires no additional configuration on OpenRouter ^[001a-raw/document/openrouter/openrouter-070-guides-best-practices-prompt-caching-2026-04-29.md]
- Cache writes: no cost ^[001a-raw/document/openrouter/openrouter-070-guides-best-practices-prompt-caching-2026-04-29.md]
- Cache reads: charged at a reduced multiplier of the original input pricing ^[001a-raw/document/openrouter/openrouter-070-guides-best-practices-prompt-caching-2026-04-29.md]

## Related

- [[004-wiki/entities/openrouter]]
- [[004-wiki/concepts/prompt-caching]]
- [[004-wiki/concepts/provider-sticky-routing]]