...
0. **Check source size and approval** (pre-processing)
   - If the file is a PDF, binary document, or exceeds ~3,000 words: invoke the document-processor agent to run the full pipeline (document-converter → ocr-remediator → markdown-chunker → auto-remediation)
