...
| `ANTHROPIC_VERTEX_PROJECT_ID`  | GCP project ID for Vertex AI. Required when using [Google Vertex AI](https://code.claude.com/docs/en/google-vertex-ai)  |  
...
| `CLAUDE_CODE_ENABLE_TASKS`  | Set to `1` to enable the task tracking system in non-interactive mode (the `-p` flag). Tasks are on by default in interactive mode. See [Task list](https://code.claude.com/docs/en/interactive-mode#task-list)  |  
...
| `CLAUDE_CODE_FORK_SUBAGENT`  | Set to `1` to enable [forked subagents](https://code.claude.com/docs/en/sub-agents#fork-the-current-conversation). A forked subagent inherits the full conversation context from the main session instead of starting fresh. When enabled, `/fork` spawns a forked subagent rather than acting as an alias for [`/branch`](https://code.claude.com/docs/en/commands), and all subagent spawns run in the background. Works in interactive mode and via the SDK or `claude -p`  |  
...
| `CLAUDE_CODE_GLOB_HIDDEN`  | Set to `false` to exclude dotfiles from results when Claude invokes the [Glob tool](https://code.claude.com/docs/en/tools-reference). Included by default. Does not affect `@` file autocomplete, `ls`, Grep, or Read  |  
...
| `CLAUDE_CODE_SCROLL_SPEED`  | Set the mouse wheel scroll multiplier in [fullscreen rendering](https://code.claude.com/docs/en/fullscreen#mouse-wheel-scrolling). Accepts values from 1 to 20. Set to `3` to match `vim` if your terminal sends one wheel event per notch without amplification  |  
...
| `CLAUDE_CODE_SIMPLE`  | Set to `1` to run with a minimal system prompt and only the Bash, file read, and file edit tools. MCP tools from `--mcp-config` are still available. Disables auto-discovery of hooks, skills, plugins, MCP servers, auto memory, and CLAUDE.md. The [`--bare`](https://code.claude.com/docs/en/headless#start-faster-with-bare-mode) CLI flag sets this  |  
...
| `FALLBACK_FOR_ALL_PRIMARY_MODELS`  | Set to any non-empty value to trigger fallback to [`--fallback-model`](https://code.claude.com/docs/en/cli-reference#cli-flags) after repeated overload errors on any primary model. By default, only Opus models trigger the fallback  |  
...
| `OTEL_LOG_TOOL_DETAILS`  | Set to `1` to include tool input arguments, MCP server names, raw error strings on tool failures, and other tool details in OpenTelemetry traces and logs. Disabled by default to protect PII. See [Monitoring](https://code.claude.com/docs/en/monitoring-usage)  |  
...
## 
[​](https://code.claude.com/docs/en/env-vars#see-also)
See also
...
Terms and policies
[Privacy choices](https://code.claude.com/docs/en/env-vars)[Privacy policy](https://www.anthropic.com/legal/privacy)[Disclosure policy](https://www.anthropic.com/responsible-disclosure-policy)[Usage policy](https://www.anthropic.com/legal/aup)[Commercial terms](https://www.anthropic.com/legal/commercial-terms)[Consumer terms](https://www.anthropic.com/legal/consumer-terms)
Assistant
